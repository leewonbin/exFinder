...
public class AdminController {
    ...
    @RequestMapping(value = "/admin/adminUserinfo", method = RequestMethod.GET)
	public void adminUserInfo(Model model, HttpSession session) throws Exception {
    	String userid = (String)session.getAttribute("userId");
		session.setAttribute("dto", userService.selectUser(userid));
    ...
    public String updateStatus(@RequestParam String u_id, @RequestParam int enabled, RedirectAttributes redirectAttributes) {
        ...
        return "redirect:/admin/adminUserinfo"; // 상태 업데이트 후 다시 관리자 페이지로 리다이렉트
    }
    
    @RequestMapping(value = "/admin/adminAuthority", method = RequestMethod.GET)
	public void adminAuthority(Model model, HttpSession session) throws Exception {
    	String userid = (String)session.getAttribute("userId");
		session.setAttribute("dto", userService.selectUser(userid));
 ...
 // 권한 업데이트를 처리하는 POST 요청
    ...
    public String updateAuthority(@RequestParam String u_id, @RequestParam String authority, RedirectAttributes redirectAttributes) {
        ...
        return "redirect:/admin/adminAuthority"; // 권한 업데이트 후 다시 관리자 페이지로 리다이렉트
    }
    
    @RequestMapping(value = "/admin/adminCs", method = RequestMethod.GET)
	public void adminCs(Model model, HttpSession session) throws Exception {
    	String userid = (String)session.getAttribute("userId");
		session.setAttribute("dto", userService.selectUser(userid));
	...
	public void adminBoard(Model model, HttpSession session) throws Exception {
    	String userid = (String)session.getAttribute("userId");
		session.setAttribute("dto", userService.selectUser(userid));
    ...
    public String deleteBoardAdmin(@RequestParam("b_id") int b_id,RedirectAttributes redirectAttributes) throws Exception  {
        ...
        return "redirect:/admin/adminBoard";  // 게시판 목록으로 리다이렉트
    }

    @RequestMapping(value = "/admin/adminUpdateEx", method = RequestMethod.GET)
	public void adminUpdateEx(Model model, HttpSession session) throws Exception {
    	String userid = (String)session.getAttribute("userId");
		session.setAttribute("dto", userService.selectUser(userid));
